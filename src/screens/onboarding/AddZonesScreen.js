...
const AddZonesScreen = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const zonesRes = await apiService.get(API_ENDPOINTS.ZONES_ALL);
      if (zonesRes.success) {
    ...
    try {
      // Fetch user's selected zones
      const myZonesRes = await apiService.get(API_ENDPOINTS.MY_ZONES);
      if (myZonesRes.success) {
        setMyZones(myZonesRes.data || []);
      }
    } catch (error) {
      // Handle pending approval or other errors gracefully
      console.log('My zones not available yet:', error.message);
      setMyZones([]);
    }
  ...
  const isZoneAdded = (zoneId, subZoneId = null) => {
    if (subZoneId) {
      return myZones.some((z) => (z.zone_id || z.zoneId) === zoneId && (z.sub_zone_id || z.subZoneId) === subZoneId);
    }
    return myZones.some((z) => (z.zone_id || z.zoneId) === zoneId && !(z.sub_zone_id || z.subZoneId));
  };

  const getMyZone = (zoneId, subZoneId = null) => {
    if (subZoneId) {
      return myZones.find((z) => (z.zone_id || z.zoneId) === zoneId && (z.sub_zone_id || z.subZoneId) === subZoneId);
    }
    return myZones.find((z) => (z.zone_id || z.zoneId) === zoneId && !(z.sub_zone_id || z.subZoneId));
  };
          ...
          <View className="px-4 mt-4">
            ...
            <View className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              {myZones.map((zone, index) => {
                ...
                const subZoneId = zone.sub_zone_id || zone.subZoneId;
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        handleAddZone(
                          ...
                          subZoneId ? { id: subZoneId, name: subZoneName } : null
                        )
                      }
                      disabled={saving}
                    >
